...
//            Eigen::Matrix<T, 3, 3> Rbw = Qwb.conjugate().toRotationMatrix();
            ...
            Eigen::Matrix<T, 3, 1> uv_homo = projection_matrix.template block<3, 3>(0, 0) * p_cam +
                                             projection_matrix.template block<3, 1>(0, 3);
        ...
        inline static
        ceres::AutoDiffCostFunction<EventAutoDiffFactor, 1, 7>*
        create(const Point& p_w, const TimeSurface::Ptr& time_surface, int wx, int wy)
        {
            return new ceres::AutoDiffCostFunction<EventAutoDiffFactor, 1, 7>(new EventAutoDiffFactor(p_w, time_surface, wx, wy));
        }

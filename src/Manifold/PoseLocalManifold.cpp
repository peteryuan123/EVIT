...
int PoseLocalManifold::AmbientSize() const { return 7; }

int PoseLocalManifold::TangentSize() const { return 6; }
...
bool PoseLocalManifold::Plus(const double *x, const double *delta, double *x_plus_delta) const {
  ...
  x_plus_delta[kW] = q_delta_w * x[kW] - q_delta_x * x[kX] - q_delta_y * x[kY] - q_delta_z * x[kZ];
  x_plus_delta[kX] = q_delta_x * x[kW] + q_delta_w * x[kX] + q_delta_z * x[kY] - q_delta_y * x[kZ];
  x_plus_delta[kY] = q_delta_y * x[kW] - q_delta_z * x[kX] + q_delta_w * x[kY] + q_delta_x * x[kZ];
  x_plus_delta[kZ] = q_delta_z * x[kW] + q_delta_y * x[kX] - q_delta_x * x[kY] + q_delta_w * x[kZ];

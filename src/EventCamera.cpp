...
EventCamera::EventCamera(std::string config_path)
    : width_(0),
      ...
      K_inv_(Eigen::Matrix3d::Zero()),
      P_(Eigen::Matrix<double, 3, 4>::Zero()),
      distortion_parameter_(cv::Mat()),
      undistortion_map1_(cv::Mat()),
      undistortion_map2_(cv::Mat()),
      Tbc_(Eigen::Matrix4d::Identity()),
      Tcb_(Eigen::Matrix4d::Identity()) {
  ...
  if (!fs.isOpened())
    ...
    if (fs["Rbc"].isNone()) LOG(ERROR) << "config: Rbc is not set";
    cv::Mat Rbc = fs["Rbc"].mat();
    Eigen::Matrix3d Rbc_eigen;
    cv::cv2eigen(Rbc, Rbc_eigen);

    if (fs["tbc"].isNone()) LOG(ERROR) << "config: tbc is not set";
    ...
    if (fs["K"].isNone()) LOG(ERROR) << "config: K is not set";
    cv::Mat K = fs["K"].mat();
    cv::cv2eigen(K, K_);

    if (fs["height"].isNone()) LOG(ERROR) << "config: height is not set";
    height_ = fs["height"];

    if (fs["width"].isNone()) LOG(ERROR) << "config: width is not set";
    width_ = fs["width"];

    if (fs["distortion"].isNone()) LOG(ERROR) << "config: distortion is not set";
    distortion_parameter_ = fs["distortion"].mat();

    if (fs["distortion_model"].isNone()) LOG(ERROR) << "config: distortion_model is not set";

...
double IntegrationBase::GYR_N = 0.0;
double IntegrationBase::ACC_W = 0.0;
...
void IntegrationBase::setCalib(std::string config_path) {
  cv::FileStorage fs(config_path, cv::FileStorage::READ);
  LOG(INFO) << "-------------- IMU --------------";

  if (!fs.isOpened()) LOG(FATAL) << "config not open:" << config_path;

  if (fs["acc_n"].isNone()) LOG(ERROR) << "config: acc_n is not set";
  ACC_N = fs["acc_n"];

  if (fs["acc_w"].isNone()) LOG(ERROR) << "config: acc_w is not set";
  ACC_W = fs["acc_w"];

  if (fs["gyr_n"].isNone()) LOG(ERROR) << "config: gyr_n is not set";
  GYR_N = fs["gyr_n"];

  if (fs["gyr_w"].isNone()) LOG(ERROR) << "config: gyr_w is not set";
  GYR_W = fs["gyr_w"];

  if (fs["g_norm"].isNone()) LOG(ERROR) << "config: g_norm is not set";
  G.z() = fs["g_norm"];

  LOG(INFO) << "acc_n:" << ACC_N;
  LOG(INFO) << "acc_w:" << ACC_W;
  LOG(INFO) << "gyr_n:" << GYR_N;
  LOG(INFO) << "gyr_w:" << GYR_W;

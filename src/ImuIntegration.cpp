...
Eigen::Matrix<double, 15, 1> IntegrationBase::evaluate(const Eigen::Vector3d &Pi, const Eigen::Quaterniond &Qi,
                                                       ...
                                                       const Eigen::Vector3d &Vj, const Eigen::Vector3d &Baj, const Eigen::Vector3d &Bgj)
{
...
void IntegrationBase::setCalib(std::string config_path)
{
    ...
    if (fs["acc_n"].isNone())
        LOG(ERROR) << "config: acc_n is not set";
    ACC_N = fs["acc_n"];

    if (fs["acc_w"].isNone())
        LOG(ERROR) << "config: acc_w is not set";
    ACC_W = fs["acc_w"];

    if (fs["gyr_n"].isNone())
        LOG(ERROR) << "config: gyr_n is not set";
    GYR_N = fs["gyr_n"];

    if (fs["gyr_w"].isNone())
        LOG(ERROR) << "config: gyr_w is not set";
    GYR_W = fs["gyr_w"];

    if (fs["g_norm"].isNone())
        LOG(ERROR) << "config: g_norm is not set";
    G.z() = fs["g_norm"];

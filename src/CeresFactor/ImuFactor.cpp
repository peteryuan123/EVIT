...
IMUFactor::IMUFactor(CannyEVIT::IntegrationBase *_pre_integration)
:pre_integration(_pre_integration)
{
}

bool IMUFactor::Evaluate(const double *const *parameters, double *residuals, double **jacobians) const
{
...
///                ROS_BREAK();
        ...
        if (jacobians[0])
        {
            ...
            jacobian_pose_i.block<3, 3>(O_P, O_R) = Utility::skewSymmetric(Qi.inverse() * (0.5 * IntegrationBase::G * sum_dt * sum_dt + Pj - Pi - Vi * sum_dt));
            ...
            jacobian_pose_i.block<3, 3>(O_V, O_R) = Utility::skewSymmetric(Qi.inverse() * (IntegrationBase::G * sum_dt + Vj - Vi));

...
EventFactor::EventFactor(const CannyEVIT::Point &p_w,
                         const TimeSurface::Ptr &time_surface,
                         int wx,
                         int wy,
                         TimeSurface::PolarType polar,
                         TimeSurface::FieldType field_type,
                         double weight)
    : polar_(polar),
      field_type_(field_type),
      p_w_(p_w),
      time_surface_(time_surface),
      wx_(wx),
      wy_(wy),
      weight_(weight) {
...
bool EventFactor::Evaluate(const double *const *parameters, double *residuals, double **jacobians) const {
  ...
  residual = weight_ * time_surface_->evaluate(p_w_, Qi, Pi, wx_, wy_, polar_, field_type_);

  if (jacobians && jacobians[0]) {
    Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>
        jacobian_pose_i(jacobians[0], num_residuals(), 7);

    jacobian_pose_i.setZero();

    jacobian_pose_i.block(0, 0, num_residuals(), 6) =
        weight_ * time_surface_->df(p_w_, Qi, Pi, wx_, wy_, polar_, field_type_);
  }
  return true;
}